...
public class WaterMiniWidgetProvider extends AppWidgetProvider {
    ...
    static void updateWidget(Context context, AppWidgetManager appWidgetManager, int id) {
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_water_mini);

        int current = prefs.getWaterMl();
        int goal = prefs.getWaterGoal();
        int percent = (goal > 0) ? (current * 100 / goal) : 0;
        
        views.setTextViewText(R.id.widgetWaterPercent, percent + "%");

        // Кнопка +250мл
        Intent addWaterIntent = new Intent(context, ActionReceiver.class);
        addWaterIntent.setAction(ActionReceiver.ACTION_ADD_WATER);
        PendingIntent waterPi = PendingIntent.getBroadcast(context, 10, addWaterIntent, PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
        views.setOnClickPendingIntent(R.id.btnPlusWater, waterPi);

        // Клик по фону
        Intent mainIntent = new Intent(context, MainActivity.class);
        PendingIntent mainPi = PendingIntent.getActivity(context, 11, mainIntent, PendingIntent.FLAG_IMMUTABLE);
