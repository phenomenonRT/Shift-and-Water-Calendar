...
public class ClockWorkWidget4x1Provider extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int id) {
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_clock_shift_4x1);
        String text = todayType.isWork() ? todayType.getIcon() + " Смена сегодня" :
                (daysToWork == 1) ? todayType.getIcon() + " Смена завтра" :
                        (daysToWork == -1) ? "Нет смен" :
                                todayType.getIcon() + " Через " + daysToWork + " " + getDayString(daysToWork);

        views.setTextViewText(R.id.widgetStatus, text);
