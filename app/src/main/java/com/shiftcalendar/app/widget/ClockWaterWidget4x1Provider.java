...
public class ClockWaterWidget4x1Provider extends AppWidgetProvider {
    ...
    static void updateAppWidget(Context context, AppWidgetManager appWidgetManager, int id) {
        ...
        RemoteViews views = new RemoteViews(context.getPackageName(), R.layout.widget_clock_water_4x1);
        String text = (daysToWater == 0) ? "💧 Вода СЕГОДНЯ!" :
                (daysToWater == 1) ? "💧 Вода завтра" :
                        (daysToWater == -1) ? "💧 Нет данных" :
                                "💧 Через " + daysToWater + " " + getDayString(daysToWater);
